...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "sw_full_count", &chip->max_sw_full_count);
	if (rc) {
		chip->max_sw_full_count = 5;
	}
...
/*Jun.Wei@RM.BSP.CHG.Basic, 2019/08/21, check plugin temp in normal 16-41C*/
static void check_plugin_temp_status(struct oppo_chg_chip *chip)
{
	if (chip->temperature > 160 && chip->temperature < 410) {
		chip->plugin_temp_normal = 1;
	} else {
		chip->plugin_temp_normal = 0;
	}
	chg_err("Charger in temp=%d, plugin_temp_normal=%d\n", chip->temperature, chip->plugin_temp_normal);
}
...
static int oppo_chg_check_sw_full(struct oppo_chg_chip *chip)
{
	...
	if (chip->batt_volt > vbatt_full_vol_sw) {
		if (chip->icharging < 0 && (chip->icharging * -1) <= chip->limits.iterm_ma) {
			chip->sw_full_count++;
			if (chip->sw_full_count > chip->max_sw_full_count) {
				chip->sw_full_count = 0;
				chip->sw_full = true;
			}
		} else if (chip->icharging >= 0) {
			chip->sw_full_count++;
			if (chip->sw_full_count > (chip->max_sw_full_count) * 2) {
				chip->sw_full_count = 0;
				chip->sw_full = true;
...
/*Jun.Wei@RM.BSP.CHG.Basic, 2019/08/21, when plugin temp not in 16-41C never allow vooc charging current up to 6A*/
bool oppo_chg_get_plugin_temp_status(void)
{
    if (!g_charger_chip) {
        return 0;
    } else {
    	return g_charger_chip->plugin_temp_normal;
    }
}
