...
*/
		...
		mod_timer(&chip->watchdog, jiffies+msecs_to_jiffies(25000));
                if (chip->disable_adapter_output == true) {
                		...
                		((!((chip->support_vooc_by_normal_charger_path) &&  (chip->fast_chg_type == CHARGER_SUBTYPE_FASTCHG_VOOC))) && 
                		(oppo_chg_get_cool_down_status() == 1)) || oppo_chg_get_plugin_temp_status() == 0) {
                        ret_info = 0x1;
                } else {
                        ret_info = 0x2;
                }
